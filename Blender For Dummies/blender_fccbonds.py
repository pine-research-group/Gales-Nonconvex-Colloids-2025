...
for pos in pos_spheres:
    ...
    nbrm_ndx = np.where(np.linalg.norm(pos_spheres-pos) <= max_bond)
    nbrs_max = pos_spheres[nbrm_ndx]
    nbr_ndx = np.where(np.linalg.norm(nbrs_max-pos) > min_bond)
    nbrs = nbrs_max[nbr_ndx]
